...
#define NPC_D_DPS_MSGID_EVENT_E2_FILE_SPEED_CTL					0x022A				//播放速度控制(快进与慢放)（与设备能力关联）
...
#ifdef NPC_D_TDKS
#define NPC_D_DPS_MSGID_MEDIA_QV_PACKET                         0x0813              //直接发送CPacket包
#endif
#define NPC_D_DPS_MSGID_EVENT_E9_CHS_TIMER_SNAPSHOT				0x0901				//定时抓拍

//------------------------------------------------------------------------------
//回放速度
#define NPC_D_DPS_PB_DEFAULT									0x0000			//正常速度
#define NPC_D_DPS_PB_FAST_TWO									0x0001			//2倍速快进
#define NPC_D_DPS_PB_FAST_FOUR									0x0002			//4倍速快进
#define NPC_D_DPS_PB_FAST_EIGTH									0x0003			//8倍速快进
#define NPC_D_DPS_PB_FAST_SIXTEEN								0x0004			//16倍速快进
#define NPC_D_DPS_PB_FAST_THIRTYTWO								0x0005			//32倍速快进

#define NPC_D_DPS_PB_SLOW_TWO									0x0011			//2倍速慢放
#define NPC_D_DPS_PB_SLOW_FOUR									0x0012			//4倍速慢放
#define NPC_D_DPS_PB_SLOW_EIGTH									0x0013			//8倍速慢放
#define NPC_D_DPS_PB_SLOW_SIXTEEN								0x0014			//16倍速慢放
#define NPC_D_DPS_PB_SLOW_THIRTYTWO								0x0015			//32倍速慢放
...
typedef struct _NPC_S_DPS_MSG_BODY_EVENT_E2_FILE_READ
{
	...
	NPC_SHORT										o_nPBSpeed;						//回放速度：NPC_D_DPS_PB_*
...
//播放速度控制(快进与慢放)时间方式)
typedef struct _NPC_S_DPS_MSG_BODY_EVENT_E2_FILE_SPEED_CTL
{
	NPC_DWORD										i_dwFileReadId;					//文件读取ID
	NPC_INT											i_iCtlType;						//操作类型：0=恢复正常速度，1为快放，-1为慢放类型：
	NPC_INT											i_iCtlLevel;					//控制倍速：查看定义NPC_D_MON_CSD_PLAYBACK_*
} NPC_S_DPS_MSG_BODY_EVENT_E2_FILE_SPEED_CTL, *PNPC_S_DPS_MSG_BODY_EVENT_E2_FILE_SPEED_CTL;
...
typedef struct _NPC_S_DPS_MSG_BODY_EVENT_E2_EX_ALARM_INFO
{
	...
	NPC_INT											i_iAlarmEvent;					//报警事件，参见NPC_D_MON_CSD_ALARM_EVENT_*定义
	NPC_CHAR										i_sAlarmInfo[128];				//报警信息
	NPC_S_TD_DATE_TIME								i_tAlarmTime;					//报警时间
...
//定时抓拍
typedef struct _NPC_S_DPS_MSG_BODY_EVENT_E9_CHS_TIMER_SNAPSHOT
{
	NPC_DWORD									i_dwSnapshotId;						//抓拍ID
	NPC_INT										i_iChNo;							//抓拍通道
	
	NPC_CHAR									o_sPicUrl[256];						//图片保存的URL	
	NPC_CHAR									o_sThumUrl[256];					//缩略图的URL
	
	NPC_BYTE									io_sReserve[256];					//备用	
} NPC_S_DPS_MSG_BODY_EVENT_E9_CHS_TIMER_SNAPSHOT, *PNPC_S_DPS_MSG_BODY_EVENT_E9_CHS_TIMER_SNAPSHOT;
...
//-------------------------------------------------------------------------------
//音/视频帧数据帧消息头
typedef struct _NPC_S_DPS_MSG_BODY_MEDIA_QV_PACKET
{
	NPC_USHORT										i_usChNo;						//通道号
	NPC_USHORT										i_usStreamNo;					//码流号，0：主码流，1：子码流
    PNPC_VOID                                       i_pPacket;
} NPC_S_DPS_MSG_BODY_MEDIA_QV_PACKET, *PNPC_S_DPS_MSG_BODY_MEDIA_QV_PACKET;
#endif
